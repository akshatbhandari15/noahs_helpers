...
class Player7(Player):
    def __init__(
        ...
        self.last_seen_ark_animals: set[tuple[int, int]] = set()  # track last seen ark animals
    ...
    def check_surroundings(self, snapshot: HelperSurroundingsSnapshot) -> int:
        ...
        if snapshot.ark_view:
            current_ark_animals = {(a.species_id, a.gender.value) for a in snapshot.ark_view.animals}
    ...
    def get_action(self, messages: list[Message]) -> Action | None:
        ...
        for msg in messages:
            ...
            # gather neighbors
            neighbor_ids = {helper_info.id for cell in self.sight 
                            for helper_info in cell.helpers if helper_info.id != self.id}

            # forward message if not already sent and neighbor exists besides sender
            if msg.contents not in self.messages_sent and any(n != msg.from_helper.id for n in neighbor_ids):
        ...
        if priority_animals:
            ...
            remove_msg = encode_message(chosen.species_id, chosen.gender.value, from_ark=False)
            heapq.heappush(self.messages_to_send, remove_msg)
